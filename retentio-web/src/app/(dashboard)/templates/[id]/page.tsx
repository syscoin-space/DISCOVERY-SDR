...
export default function EditTemplatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (template && !initialized) {
      ...
      if (template.channel === "LIGACAO") {
        try {
          const parsed = JSON.parse(template.body);
          setAbertura(parsed.abertura || "");
          setDiagnostico(parsed.diagnostico || "");
          setImplicacao(parsed.implicacao || "");
          setConvite(parsed.convite || "");
          setBody(template.body);
        } catch {
          setBody(template.body);
          setAbertura(template.body);
        }
      } else {
        setBody(template.body);
      }
        ...
        <div className="mx-auto max-w-4xl flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="flex justify-end gap-3 pb-8">
              ...
              <button
                type="submit"
                disabled={!name.trim() || updateTemplate.isPending || (channel === "LIGACAO" ? (!abertura || !diagnostico || !implicacao || !convite) : !body.trim())}
