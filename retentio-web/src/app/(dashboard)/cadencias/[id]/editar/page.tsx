...
let nextId = 1;
function makeStep(dayOffset: number, channel: StepChannel = "EMAIL", instructions = "", template_id = ""): StepDraft {
  return { id: `draft-${nextId++}`, day_offset: dayOffset, channel, instructions, template_id };
}
...
export default function EditarCadenciaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (cadence && !initialized) {
      ...
      setSteps(
        (cadence.steps ?? []).map((s) =>
          makeStep(s.day_offset, s.channel, s.instructions ?? "", s.template_id ?? "")
        )
      );
      setInitialized(true);
    }
